using DE.Application.DTOs.Response;
using DE.Application.Interfaces;
using DE.Application.Interfaces.Services;
using Mapster;

namespace DE.Application.Services;

public class CountryService(IUnitOfWork uow) : ICountryService
{
    private readonly IUnitOfWork _uow = uow;

    public async Task<IEnumerable<CountryResponse>> GetAllAsync()
    {
        var result = await _uow.CountryRepository.GetAllAsync();
        return result.Adapt<IEnumerable<CountryResponse>>();
    }
}
